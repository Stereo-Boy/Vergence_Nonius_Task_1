...
TextTable.fig1.subfig2.en={'Estimation by gaussian fit','p(Success)','log - Offset (arcmin)'};
TextTable.fig1.subfig3.en={'Estimation by raw data','p(Success)','log - Offset (arcmin)'};
...
for i=1:nbCond 
        subplot(3,2,3)
        coloriz=colors(1,i);
        line([stairs.thresLine(i,1), stairs.thresLine(i,1)],[0 1],'color',coloriz,'LineStyle','--')
        hold on
        plot(stairs.valueClasses(i,:,1),stairs.valueClasses(i,:,2),'-o','color',coloriz)
        plot(xx,stairs.yg1(i,:),'-','color',coloriz)
        line([stairs.thresLine(i,2), stairs.thresLine(i,2)],[0 1],'color',coloriz,'LineStyle','-')
        legendAxis(TextTable,1,2,'en',fontSize)  ;

        subplot(3,2,5)
        plot(stairs.values{i},stairs.dataResp{i},'o','color',coloriz)
        hold on
        plot(xx,stairs.yg2(i,:),'-','color',coloriz)
        line([stairs.thresLine(i,3), stairs.thresLine(i,3)],[0 1],'color',coloriz,'LineStyle','-')
...
for i=1:nbCond
    disp(['Condition',num2str(i)])
     for j=1:nbLevel  
        ...
        hold on
        coloriz=colors(j,i);
        plot(squeeze(stairs.lvlClasses(i,j,:,1)),squeeze(stairs.lvlClasses(i,j,:,2)),':o','color',coloriz)
        plot(xx,squeeze(stairs.yg1lvl(i,j,:)),'-','color',coloriz)
        line([stairs.thresLvlLine(i,j,2), stairs.thresLvlLine(i,j,2)],[0 1],'color',coloriz,'LineStyle',':')
        legendAxis(TextTable,1,2,'en',fontSize)  ;

        subplot(3,2,6)
        plot(stairs.values{i},stairs.dataResp{i},'o','color',coloriz)
        hold on
        plot(xx,squeeze(stairs.yg2lvl(i,j,:)),'-','color',coloriz)
        line([stairs.thresLvlLine(i,j,3), stairs.thresLvlLine(i,j,3)],[0 1],'color',coloriz,'LineStyle',':')
...
for i=1:nbCond
       ...
       figure(2)
        coloriz=colors(1,i);
         plot(x1,y1,'o-','color',coloriz)
        hold on
        plot(xx,yG,'-','color',coloriz)
        line([theThres, theThres],[0 1],'color',coloriz,'LineStyle',':')
        fprintf('Threshold using fitting function = %5.3f arcsec.\n', 60.*exp(theThres));
        %fprintf('Threshold using raw data fit = (%5.3f).\n', 10.^theThres);
        legendAxis(TextTable,1,2,'en',fontSize)  ;
end
...
for i=1:nbCond
       ...
       subplot(1,2,i)
        coloriz=colors(1,i);
         ...
         %then split the data for each condition in groups with equal numbers of data
          list1=list(list(:,13)==1,:); %right eye upper line
          list2=list(list(:,13)==-1,:); %left eye upper line
        ...
        plot(x1,y1,'x-','color',coloriz)
        
        plot(xx,yG,'-','color',coloriz)
        line([bias, bias],[0 1],'color',coloriz,'LineStyle',':')
        plot(xx,yG1,':','color',coloriz)
        line([bias1, bias1],[0 1],'color',coloriz,'LineStyle','-')
        plot(xx,yG2,':','color',coloriz)
        line([bias2, bias2],[0 1],'color',coloriz,'LineStyle','-')
        disp(['Threshold using fitting function = ',num2str(60.*(theThres)),'  arcsec.']);
        disp(['Threshold using fitting function = ',num2str(60.*(theThres1)),' arcsec.']);
        disp(['Threshold using fitting function = ',num2str(60.*(theThres2)),' arcsec.']);
        disp(['Bias using fitting function = ',num2str(60.*(bias)),'  arcsec.']);
        disp(['Bias using fitting function = ',num2str(60.*(bias1)),' arcsec.']);
        disp(['Bias using fitting function = ',num2str(60.*(bias2)),' arcsec.']);
        %fprintf('Threshold using raw data fit = (%5.3f).\n', 10.^theThres);
        legendAxis(TextTable,3,i,'en',fontSize)  ;
        divBias=bias2-bias1;
        disp(['Bias (negative is over-convergence) = ',num2str(60.*mean([divBias])),' arcsec.']);
        disp(['Mean after bias correction = ',num2str(60.*mean([theThres1,theThres2])),' arcsec.']);
end
